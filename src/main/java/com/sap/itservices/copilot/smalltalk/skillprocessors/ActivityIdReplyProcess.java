...
@RecastSkill({ "show-my-activityid", "activityid" })
public class ActivityIdReplyProcess implements SkillProcessor {

	private static final Logger LOGGER = LoggerFactory.getLogger(ActivityIdReplyProcess.class);
	...
	public void process(AIRequest request, AIResponse response, RecastDialogResponse recastResponse) {
		...
		if (messages.size() == 0) {
			...
			if (intents.size() > 0) {
				...
				textMessage = "Siva ActivityId Actions message not configured to Recast.ai";
			}
